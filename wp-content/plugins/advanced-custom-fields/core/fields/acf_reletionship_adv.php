<?php
class acf_field_relationship_adv extends acf_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	{
		// vars
		$this->name = 'relationship_adv';
		$this->label = __("Relationship Advanced",'acf');
    	...
    	// extra
		add_action('wp_ajax_acf/fields/relationship_adv/query_posts', array($this, 'query_posts'));
		add_action('wp_ajax_nopriv_acf/fields/relationship_adv/query_posts', array($this, 'query_posts'));
	}
	...
	function query_posts()
   	{
		...
		// filters
		$options = apply_filters('acf/fields/relationship_adv/query', $options, $field, $the_post);
		$options = apply_filters('acf/fields/relationship_adv/query/name=' . $field['_name'], $options, $field, $the_post );
		$options = apply_filters('acf/fields/relationship_adv/query/key=' . $field['key'], $options, $field, $the_post );
		...
		while( $wp_query->have_posts() )
		{
			...
			// filters
			$title = apply_filters('acf/fields/relationship_adv/result', $title, $post, $field, $the_post);
			$title = apply_filters('acf/fields/relationship_adv/result/name=' . $field['_name'] , $title, $post, $field, $the_post);
			$title = apply_filters('acf/fields/relationship_adv/result/key=' . $field['key'], $title, $post, $field, $the_post);
	...
	<div class="relationship_right">
		...
		if( $field['value'] )
		{
			foreach( $field['value'] as $p )
			{
				...
				// filters
				$title = apply_filters('acf/fields/relationship_adv/result', $title, $p, $field, $post);
				$title = apply_filters('acf/fields/relationship_adv/result/name=' . $field['_name'] , $title, $p, $field, $post);
				$title = apply_filters('acf/fields/relationship_adv/result/key=' . $field['key'], $title, $p, $field, $post);
...
new acf_field_relationship_adv();
