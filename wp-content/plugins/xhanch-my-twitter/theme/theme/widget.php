<?php
	...
	foreach($res as $sts_id=>$row){
		echo '<li>';
			...
			$tmp_str = str_replace('@screen_name_plain', '<strong class="title">'.$row['author'].'</strong>', $twt_lyt);
			...
			$tmp_str = str_replace('@tweet', wpautop($row['tweet'],false), $tmp_str);
			...
			$tmp_str = str_replace('@readmore', '<a href="http://twitter.com/'.$xmt_acc[$acc]['cfg']['twt_usr_nme'].'" class="more">'.__('Read More','xmt').'</a>', $tmp_str);
